  ...
  return (
    <section id="experience" className="py-20 bg-slate-900/50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold text-slate-100 mb-4">Professional Experience</h2>
        ...
        >
          {EXPERIENCE.map((exp, expIndex) => (
            <div key={expIndex} className="relative">
              <div className="absolute -left-[21px] top-0 w-4 h-4 rounded-full bg-neon-400 shadow-[0_0_10px_#22d3ee]" />
              ...
              <div className="bg-slate-800/30 backdrop-blur-md p-8 rounded-2xl border border-slate-700 hover:border-neon-400/50 transition-all duration-300">
                <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-6">
                  <div>
                    <h3 className="text-3xl font-bold text-slate-100 flex items-center gap-2">
                      <Briefcase className="text-neon-400 w-5 h-5" />
                      {exp.role}
                    </h3>
                    <h4 className="text-2xl text-neon-400 mt-1">{exp.company}</h4>
                  </div>
                ...
                <ul className="space-y-3">
                  {exp.description.map((item, index) => (
                    <li key={index} className="flex items-start text-xl text-slate-300">
